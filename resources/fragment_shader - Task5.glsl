...
float getShadow(vec3 pt){
    ...
    // march ray towards each light source, don't illuminae if SDF drops too close to 0
    for (int i=0; i< LIGHT_POS.length(); i++){
        vec3 lightDir = normalize(LIGHT_POS[i] - pt);

        // shadow code from lec notes
        for (float t = 0.1;
         ...
         && step < RENDER_DEPTH && kd > 0.001; ) {
            ...
            if (d < CLOSE_ENOUGH){
                ...
                // divide d by t = attenuate the strength of the shadow as its source is further from the illumination point
                kd = min(kd, 16 * d / t);
            }
            t += d;
            step++;
